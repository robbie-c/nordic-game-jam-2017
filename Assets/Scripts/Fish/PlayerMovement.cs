...
public class PlayerMovement : MonoBehaviour {
	...
	public float accelerationCutoff = 0.5f;  // As the fish turn the turning speed factor goes from 0 to 1.  This cutoff indicates the minimum turning speed factor that produces the maximum acceleration.
	float turningSpeed;
	...
	void Update () {
		...
		if (Input.GetKeyDown(keyTurnRight) || Input.GetKeyDown(keyTurnLeft)) timeAfterTurnButtonDown = 0.0f;
	...
	void FixedUpdate () {
		...
		wiggleAcceleration = CalculateWiggleAcceleration (acceleration, CalculateTurningSpeedFactor(timeAfterTurnButtonDown, timeAfterTurnButtonUp), accelerationCutoff);
		
		Debug.Log("wiggleAcceleration = " + wiggleAcceleration);
		Accelerate(velocity,  wiggleAcceleration);
	...
	float CalculateWiggleAcceleration (float acceleration, float turningSpeedFactor, float threshold) {
		if (turningSpeedFactor >= threshold) {
			return acceleration;
		} else {
			return (turningSpeedFactor - threshold) * acceleration;
		}
	}
